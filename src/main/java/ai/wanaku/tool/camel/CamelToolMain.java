...
public class CamelToolMain implements Callable<Integer> {
    ...
    private ServiceTarget newToolInvokerTarget() {
        String address = DiscoveryHelper.resolveRegistrationAddress(registrationAnnounceAddress);
        return ServiceTarget.newEmptyTarget(name, address, grpcPort, ServiceType.TOOL_INVOKER);
    }

    private ServiceTarget newResourceProviderTarget() {
        String address = DiscoveryHelper.resolveRegistrationAddress(registrationAnnounceAddress);
        return ServiceTarget.newEmptyTarget(name, address, grpcPort, ServiceType.RESOURCE_PROVIDER);
    }
    ...
    public Integer call() throws Exception {
        final ServiceTarget toolInvokerTarget = newToolInvokerTarget();
        RegistrationManager toolRegistration = newRegistrationManager(toolInvokerTarget);

        final ServiceTarget resourceProviderTarget = newResourceProviderTarget();
        RegistrationManager resourceRegistration = newRegistrationManager(resourceProviderTarget);
        ...
        try {
            ...
            toolRegistration.deregister();
            resourceRegistration.deregister();
        }
